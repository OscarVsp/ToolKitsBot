...
class RoleFromRoles(RoleFromABC):
    ...
    def embed(self) -> disnake.Embed:
        return super().add_field(
            disnake.Embed(
                title="__**Role from roles Export**__",
                description="__Roles source: __\n> " + "\n> ".join([role.mention for role in self.roles]),
            )
        )

    async def end_embed(self, role: disnake.Role, interaction: disnake.MessageInteraction):
        await interaction.edit_original_response(
            embed=super().add_field(
                disnake.Embed(
                    description="Role(s) "
                    + ", ".join([role.mention for role in self.roles])
                    + f" members added to role {role.mention}"
                )
            ),
            view=None,
        )
...
class RoleFromEvent(RoleFromABC):
    ...
    def embed(self) -> disnake.Embed:
        return super().add_field(
            disnake.Embed(
                title="__**Role from event Export**__", description=f"__Event source: __\n> {self.event.name}"
            )
        )
    ...
    async def end_embed(self, role: disnake.Role, interaction: disnake.MessageInteraction):
        await interaction.edit_original_response(
            embed=super().add_field(
                disnake.Embed(description=f"Event's **{self.event.name}** participant(s) added to role {role.mention}")
            ),
            view=None,
        )
        await interaction.delete_original_response(delay=5)
...
class Export(commands.Cog):
    ...
    @commands.slash_command(
        name="role", description="Create role", default_member_permissions=disnake.Permissions.all()
    )
    async def role(self, inter: disnake.ApplicationCommandInteraction):
        pass

    @role.sub_command_group(name="from")
    async def role_from(self, inter: disnake.ApplicationCommandInteraction):
        pass

    @role_from.sub_command(name="roles")
    ...
    @role_from.sub_command(name="event")
    async def role_from_event(
        self,
