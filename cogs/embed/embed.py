...
class EmbedBase(disnake.ui.View):
    ...
    def __init__(self, interaction: disnake.ApplicationCommandInteraction):
        ...
        self.targets: List[disnake.TextChannel | disnake.VoiceChannel | disnake.NewsChannel | disnake.Thread] | List[
            disnake.User | disnake.Role
        ] = []

    @property
    def embeds(self) -> List[disnake.Embed]:
        self.editorEmbed.clear_fields()
        self.editorEmbed.add_field(
            name="__*Target(s)*__",
            value=("> " + "\n> ".join([target.mention for target in self.targets]) if self.targets else "*no target*"),
            inline=False,
        )
    ...
    async def update(
        self,
        interaction: disnake.MessageInteraction | disnake.ModalInteraction = None,
...
class EmbedCreator(EmbedBase):
    ...
    async def confirm(self, button: disnake.ui.Button, interaction: disnake.MessageInteraction):
        ...
        await self.interaction.edit_original_response(
            embed=disnake.Embed(description="Embed send !", color=disnake.Colour.green()), view=None
        )
...
class EmbedEditorMentionable(EmbedCreator):
    @disnake.ui.mentionable_select(placeholder="User(s)/Role(s)", min_values=1, max_values=25, row=3)
    async def mentionable(
        self, mentionableSelected: disnake.ui.MentionableSelect, interaction: disnake.MessageInteraction
    ):
        await interaction.response.defer()
...
class EmbedEditorChannel(EmbedCreator):
...
class EmbedEditor(EmbedBase):
    def __init__(self, interaction: disnake.ApplicationCommandInteraction, msg: disnake.Message):
        super().__init__(interaction)
        self.msg = msg
        self.cmdEmbeds = msg.embeds
        self.current_embed_n = len(msg.embeds) - 1
    ...
    async def send(self):
        await self.update(self.interaction)

    @disnake.ui.button(label="Edit message", emoji="📝", style=disnake.ButtonStyle.green, row=4)
    async def confirm(self, button: disnake.ui.Button, interaction: disnake.MessageInteraction):
        await interaction.response.defer()
        await self.msg.edit(embeds=self.cmdEmbeds)
        await self.interaction.edit_original_response(
            embed=disnake.Embed(description="Embed edited !", color=disnake.Colour.green()), view=None
        )
        await interaction.delete_original_response(delay=3)
        ...
        super().__init__(
            ...
            components=[
                disnake.ui.TextInput(
                    label='Title (required if "Description" is empty)',
                    custom_id="title",
                    value=self.embedEditor.cmdEmbeds[self.embedEditor.current_embed_n].title if edit else None,
                    required=False,
                    min_length=0,
                    max_length=256,
                ),
                disnake.ui.TextInput(
                    label='Description (required if "Title" is empty)',
                    custom_id="description",
                    value=self.embedEditor.cmdEmbeds[self.embedEditor.current_embed_n].description if edit else None,
                    required=False,
                    min_length=0,
                    max_length=4000,
                ...
                disnake.ui.TextInput(
                    label="Json import (overwrite the entire embed!)",
                    custom_id="json",
                    required=False,
                    style=disnake.TextInputStyle.paragraph,
                ),
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        ...
        if interaction.text_values.get("json"):
            try:
                ...
                await self.embedEditor.update(interaction=interaction, warning_message="Invalide json format...")
                return
            else:
                ...
                await self.embedEditor.update(interaction=interaction)
        elif interaction.text_values.get("title") or interaction.text_values.get("description"):
            if self.edit:
                self.embedEditor.cmdEmbeds[self.embedEditor.current_embed_n].title = interaction.text_values.get(
                    "title"
                )
                self.embedEditor.cmdEmbeds[self.embedEditor.current_embed_n].description = interaction.text_values.get(
                    "description"
                )
            else:
                new_embed = disnake.Embed(
                    ...
                    color=self.embedEditor.selected_color,
                )
                self.embedEditor.cmdEmbeds.append(new_embed)
                self.embedEditor.current_embed_n = len(self.embedEditor.cmdEmbeds) - 1
            await self.embedEditor.update(interaction=interaction)
        else:
            await self.embedEditor.update(
                interaction=interaction, warning_message="Embed creation require at least `Title` or `Description`."
            )
...
class Image(disnake.ui.Modal):
    def __init__(self, embedEditor: EmbedEditor) -> None:
        ...
        super().__init__(
            ...
            components=[
                disnake.ui.TextInput(
                    label='Image (require "Title" or "Description")',
                    custom_id="image",
                    placeholder="The embed's image URL",
                    required=False,
                    value=self.embedEditor.cmdEmbeds[self.embedEditor.current_embed_n].image.url
                    if self.embedEditor.cmdEmbeds[self.embedEditor.current_embed_n].image
                    else None,
                ),
                disnake.ui.TextInput(
                    label='Thumbnail (require "Title" or "Description")',
                    custom_id="thumbnail",
                    placeholder="The thumbnail's url",
                    required=False,
                    value=self.embedEditor.cmdEmbeds[self.embedEditor.current_embed_n].thumbnail.url
                    if self.embedEditor.cmdEmbeds[self.embedEditor.current_embed_n].thumbnail
                    else None,
                ),
            ],
        )
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        ...
        self.embedEditor.cmdEmbeds[self.embedEditor.current_embed_n].set_thumbnail(
            url=interaction.text_values.get("thumbnail")
        )
        await self.embedEditor.update(interaction=interaction)
...
class Detail(disnake.ui.Modal):
        ...
        super().__init__(
            ...
            components=[
                disnake.ui.TextInput(
                    ...
                    value=self.embedEditor.cmdEmbeds[self.embedEditor.current_embed_n].author.name,
                    required=False,
                    max_length=256,
                ),
                disnake.ui.TextInput(
                    label="Author URL (require author's name)",
                    custom_id="author_url",
                    placeholder="The author's URL",
                    value=self.embedEditor.cmdEmbeds[self.embedEditor.current_embed_n].author.url,
                    required=False,
                ),
                disnake.ui.TextInput(
                    label="Author icon (require author's name)",
                    custom_id="author_icon",
                    placeholder="The author's icon URL",
                    value=self.embedEditor.cmdEmbeds[self.embedEditor.current_embed_n].author.icon_url,
                    required=False,
                ),
                disnake.ui.TextInput(
                    label="Footer text",
                    ...
                    value=self.embedEditor.cmdEmbeds[self.embedEditor.current_embed_n].footer.text,
                    required=False,
                    max_length=2048,
                ),
                disnake.ui.TextInput(
                    label="Footer icon (require Footer's text)",
                    custom_id="footer_icon",
                    placeholder="The footer's icon URL",
                    value=self.embedEditor.cmdEmbeds[self.embedEditor.current_embed_n].footer.icon_url,
                    required=False,
                ),
            ],
        )
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        ...
        await self.embedEditor.update(interaction=interaction, warning_message=warning_message)
...
class Field(disnake.ui.Modal):
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        ...
        if interaction.text_values.get("name") and interaction.text_values.get("text"):
            self.embedEditor.cmdEmbeds[self.embedEditor.current_embed_n].add_field(
                interaction.text_values.get("name"), interaction.text_values.get("text"), inline=inline
            )
        await self.embedEditor.update(interaction=interaction)
...
class Embed(commands.Cog):
    ...
    @embed.sub_command(name="channel", description="Send an embed in one/multiple channels")
    async def embed_channel(self, inter: disnake.ApplicationCommandInteraction):
        await inter.response.defer(ephemeral=True)
        EM = EmbedEditorChannel(inter)
        await EM.send()

    @embed.sub_command(name="user", description="Send an embed to users in DM")
    ...
    @commands.message_command(name="embed_export")
    async def embed_export(self, interaction: disnake.MessageCommandInteraction):
        ...
        if jsons_embed:
            ...
            await interaction.edit_original_response(
                embed=disnake.Embed(
                    description=f"*No embed to export in [this message]({interaction.target.jump_url})*"
                )
            )
